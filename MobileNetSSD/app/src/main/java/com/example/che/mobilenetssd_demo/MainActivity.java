...
public class MainActivity extends AppCompatActivity {
    ...
    private int[] ddims = {1, 3, 100, 100}; //这里的维度的值要和train model的input 一一对应
    ...
    private void initMobileNetSSD() throws IOException {
        ...
        byte[] bin = null;
        {
            ...
            //InputStream assetsInputStream = getAssets().open("EDSRx2.param.bin");// param：  网络结构文件
            InputStream assetsInputStream = getAssets().open("MobileSR.param.bin");// param：  网络结构文件
        ...
        }
        {
            ...
            //InputStream assetsInputStream = getAssets().open("EDSRx2.bin");//bin：   model文件
            //InputStream assetsInputStream = getAssets().open("MobileNetSSD_deploy.bin");//bin：   model文件
            InputStream assetsInputStream = getAssets().open("MobileSR.bin");//bin：   model文件
            int available = assetsInputStream.available();
    ...
    private void init_view() {
        ...
        Button buttonrun = (Button) findViewById(R.id.sr);
        ...
        if (resultCode == Activity.RESULT_OK) {
            switch (requestCode) {
                case USE_PHOTO:
                    ...
                    String show_text = "width: "+ddims[2]+"\nHeight: "+ddims[3];
    ...
    private void predict_image(String image_path) {
        ...
        try {
            ...
            Bitmap output_bmp = Bitmap.createScaledBitmap(rgba, result.length, result.length, false);
            show_image.setImageBitmap(output_bmp);
            String show_text = "width: "+result.length+"\nHeight: "+result.length+"\ntime： " + time + "ms" ;
            result_text.setText(show_text);
